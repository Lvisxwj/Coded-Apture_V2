...
# Shared 32-channel bounds used by training, evaluation and visualization.
VEGINDEX_MAX_VALUES = (
    3654, 640320, 0.68508, 14, 6.33333, 22.3506, 0.0010135, 0.0017025,
    2104.53, 1.13444, 15.4096, 2.44174, 0.756353, 13206, 0.779797, 1630,
    2.79939, 0.881562, 2.42226, 24288.5, 1.15107, 0.914317, 0.93173,
    90.0882, 17182.6, 7446.72, 7.2086, 0.445404, -0.162209,
    0.00155797, 1.12579, 1.08031,
)
VEGINDEX_MIN_VALUES = (
    -5770, -757080, -0.411147, -3.4, -2.2, 0.80512, -0.000382122,
    -0.000387671, -2852.4, -0.641313, -0.416895, -0.351643, -0.427581,
    -8364, -0.559531, -2980, 0.092725, -1.04776, -0.419541, -27577.2,
    -2.28623, -0.398923, -0.0343407, -41.8549, -15440.6, -8535.6,
    0.400971, -0.795348, -0.922897, -0.000430041, 0.0448032, 0.0327779,
)


def _vegindex_bounds(input_data):
    if input_data.ndim != 4 or input_data.shape[1] != 32:
        raise ValueError(
            f"Expected [batch, 32, height, width], got {tuple(input_data.shape)}"
        )
    dtype = input_data.dtype if input_data.is_floating_point() else torch.float32
    min_values = torch.as_tensor(
        VEGINDEX_MIN_VALUES, device=input_data.device, dtype=dtype
    ).view(1, 32, 1, 1)
    max_values = torch.as_tensor(
        VEGINDEX_MAX_VALUES, device=input_data.device, dtype=dtype
    ).view(1, 32, 1, 1)
...
def max_min_norm(input_data):
    ...
    """
    min_values, max_values = _vegindex_bounds(input_data)
    return (input_data - min_values) / (max_values - min_values)
...
def reverse_max_min_norm(normalized_data):
    ...
    """
    min_values, max_values = _vegindex_bounds(normalized_data)
