...
class SSIM(torch.nn.Module):
    def __init__(self, window_size=11, size_average=True, val_range=None):
        ...
        self.channel = 1
        self.window = None  # lazily created on first forward

    def forward(self, img1, img2):
        (_, channel, _, _) = img1.size()

        if self.window is None or channel != self.channel or self.window.device != img1.device or self.window.dtype != img1.dtype:
            window = create_window(self.window_size, channel).to(img1.device).type(img1.dtype)
            self.window = window
            self.channel = channel

        s_score, ssim_map = ssim(img1, img2, window=self.window, window_size=self.window_size,
